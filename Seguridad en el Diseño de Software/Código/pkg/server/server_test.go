...
func newTestHTTPServer(t *testing.T) *httptest.Server {
	...
	// Generamos la clave maestra para el servidor de test, igual que en Run().
	// Sin ella, las llamadas a crypto.Encrypt/Decrypt fallarían con clave nil.
	masterKey := make([]byte, 32)
	if _, err := rand.Read(masterKey); err != nil {
		t.Fatalf("no se ha podido generar la clave maestra de test: %v", err)
	}

	srv := &server{db: db, masterKey: masterKey}
